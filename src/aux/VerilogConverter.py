...
class VerilogConverter:
    ...
    def convert(self, file, new_file, collapse=True):
        ...
        if extension != "v":
            ...
            os.remove(config.abc_path.joinpath(file))
            ...
            shutil.copy2(str(self.abc_path) + os.path.sep + file, str(self.abc_path) + os.path.sep + new_file)
            shutil.copy2(str(self.abc_path) + os.path.sep + file, str(self.benchmark_path) + os.path.sep + new_file)
