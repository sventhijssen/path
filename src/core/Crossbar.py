...
class Crossbar(BooleanFunction):
    ...
    def __init__(self, rows: int, columns: int, layers: int = 1, default_literal=Literal("False", False)):
        ...
        self.matrix = [[[Memristor(r, c, default_literal, l) for c in range(self.columns)] for r in range(self.rows)] for l
                       in range(self.layers)]
    ...
    def find(self, literal: Literal) -> List[Tuple[int, int]]:
        """
                ...
                for r in range(self.rows):
                    for c in range(self.columns):
                        if self.get_memristor(r, c, layer=l).literal == literal:
                            positions.append((l, r, c))
        else:
            for r in range(self.rows):
                for c in range(self.columns):
                    if self.get_memristor(r, c).literal == literal:
